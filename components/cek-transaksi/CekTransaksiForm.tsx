 ...
 *
 * Dipisah dari page.tsx supaya halamannya bisa jadi komponen server dan
 * mengambil daftar metode pembayaran — komponen klien tidak bisa membaca
 * database sendiri, dan rekening tujuan HARUS ada di halaman ini. Lihat
 * migrasi 22: ini satu-satunya halaman yang bisa dibuka kapan saja tanpa
 * login, jadi pembeli yang menutup tab atau kembali besok tetap tahu ke mana
 * harus mentransfer.
 */
export default function CekTransaksiForm({ paymentMethods }: { paymentMethods: PaymentMethod[] }) {
        ...
        {result.kind === 'found' && (
          <Card variant="raised" className="rounded-[22px]">
            <CardContent className="p-6 space-y-5">
              ...
              {result.pending && (
                <>
                  {/* Rekening yang PERSIS dipilih pembeli kalau tercatat.
                      Kalau tidak (rekber tidak menyimpan metode pembayaran,
                      atau metodenya sudah dihapus admin), seluruh pilihan
                      ditampilkan — lebih baik memilih sendiri daripada tidak
                      tahu sama sekali. */}
                  <PaymentDestination
                    methods={cocokkanMetode(paymentMethods, result.paymentMethod)}
                    total={result.amount}
                  />
                  <PaymentProofUpload
